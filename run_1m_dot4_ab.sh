#!/usr/bin/env bash
# A/B the 4-accumulator rerank dot (BulkDotKernel.dot) vs core's single-accumulator uint8DotProduct
# (-Dlloyd.noSimdCentroidDot=true -> scalar fallback = core path). JFR: rerank reduceLanes ~20% from the
# single-acc dependent chain. Reuses cached index, np40/bn1750, counting-select on. Recall must be identical.
set -uo pipefail
...
run_arm() {
  local name="$1"; local opts="$2"; local log="$OUT/run_1m_dot4_${name}_${STAMP}.log"
  echo "=== [$name] $(date) ==="
...
run_arm DOT4_NEW  ""
run_arm CORE_1ACC "-Dlloyd.noSimdCentroidDot=true"
echo "=== done $(date) ==="
