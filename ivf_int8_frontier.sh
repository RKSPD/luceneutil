...
# Identical frontier config but int8 fine tier: isolates fine-tier ranking loss from coarse/routing.
...
python -u src/python/knnPerfTest.py 2>&1 | grep -iE "will now reindex|graphDescents|^SUMMARY:|mfScanned" | tail -6
echo INT8DONE
