...
# Shared frontier config. Only the fine tier changes between arms.
export KNN_HEAP=16g KNN_INDEX_TYPE=ivfaster \
...
for TIER in afterburner5 afterburner5inline; do
  echo "=== fineTier=$TIER ==="
  IVFASTER_FINE_TIER=$TIER python -u src/python/knnPerfTest.py 2>&1 | grep -iE "^SUMMARY:|udot=|reindex" | tail -4
done
echo AB5ABDONE
