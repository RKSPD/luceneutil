...
export KNN_HEAP=16g KNN_INDEX_TYPE=ivfaster IVFASTER_FINE_TIER=afterburner6 \
...
for NP in 75 80 85; do
  for R in 1 2; do
    echo "=== np=$NP rep$R ==="
    KNN_NPROBE=$NP python -u src/python/knnPerfTest.py 2>&1 | grep -iE "^SUMMARY:" | tail -1
  done
done
echo AB6KNEEDONE
