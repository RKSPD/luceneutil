...
export KNN_HEAP=16g KNN_INDEX_TYPE=ivfaster IVFASTER_FINE_TIER=afterburner5 \
  ...
  IVFASTER_MF_QUERY_CLIP_Q=0.95 IVFASTER_UDOT_LIB=/tmp/libivfasterudot.so IVFASTER_FUSE_CELLS=1
for P in 400 450 480; do
  echo "=== threshGate permille=$P ==="
  IVFASTER_FUSE_THRESH_PERMILLE=$P python -u src/python/knnPerfTest.py 2>&1 | grep -iE "^SUMMARY:" | tail -1
done
echo FUSETHRDONE
