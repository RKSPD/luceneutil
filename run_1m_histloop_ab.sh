#!/usr/bin/env bash
# A/B the counting-select's histogram/collect loop: committed two-array version (git HEAD) vs the packed
# long[] + branchless-collect rewrite (working tree). Both run -Dlloyd.countingSelect=true on the cached 1M
# index. The BUILD differs between arms, so this script rebuilds the sandbox classes between arms.
set -uo pipefail
...
run_arm() {
  local name="$1"; local log="$OUT/run_1m_histloop_${name}_${STAMP}.log"
  echo "=== [$name] $(date) ==="
  KNN_CLEAR_CACHE=0 JAVA_TOOL_OPTIONS="-Dlloyd.countingSelect=true" ./run_knn_bench.sh 3 > "$log" 2>&1
