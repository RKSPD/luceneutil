#!/usr/bin/env bash
# Measure the cell-order postings-skip change at the golden point (nprobe=60, bulk dedup, spill=2, m1.10).
# Reuses the cached index (read-path change only). Compare recall/lat to the pre-change 0.954 @ 1.77 ms.
set -uo pipefail
cd /local/home/rikhil/vectordb/luceneutil
STAMP="$(date +%Y%m%d_%H%M%S)"; OUT=/local/home/rikhil/vectordb
LOG="$OUT/run_1m_postskip_${STAMP}.log"
...
echo "=== postings-skip, golden nprobe sweep $(date) ===" | tee "$LOG"
