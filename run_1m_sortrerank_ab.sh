#!/usr/bin/env bash
# A/B the sort-by-slot rerank reorder on the cached 1M index (search-time, no rebuild). Sorting the ~2000
# shortlist survivors by slot turns scattered code-record reads into a near-sequential sweep -> hardware
# prefetcher hides the ~80ns/record DRAM stall warm. Recall MUST be identical (pure reorder).
set -uo pipefail
...
export KNN_NPROBE="${KNN_NPROBE:-45}" KNN_NQUERY=1000 KNN_SKIP_SMELL=1 KNN_HEAP=24g
...
run_arm() {
  local name="$1"; shift; local log="$OUT/run_1m_sortrr_${name}_${STAMP}.log"
  echo "=== [$name] $(date) ==="
...
run_arm SORT_OFF JAVA_TOOL_OPTIONS=-Dlloyd.noSortRerank=true
run_arm SORT_ON  JAVA_TOOL_OPTIONS=
echo "=== done $(date) ==="
