...
run_arm() {
  local name="$1"; local opts="$2"; local log="$OUT/run_1m_hdoc_${name}_${STAMP}.log"
  echo "=== [$name] $(date) ==="
...
run_arm SEG  "-Dlloyd.countingSelect=true"
run_arm HEAP "-Dlloyd.countingSelect=true -Dlloyd.hammingHeapDocLoad=true"
echo "=== done $(date) ==="
