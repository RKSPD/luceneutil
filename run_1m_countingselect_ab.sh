#!/usr/bin/env bash
# A/B the coarse-shortlist counting-select vs the TernaryLongHeap on the cached 1M index (search-time,
# no rebuild). JFR (2026-08-06) put the heap sift at 14.4% of warm CPU. Counting-select is recall-neutral
# (keeps the same BRUTE_N min-distance set, differs only at the tie boundary). Golden config, nprobe=60.
# 3 passes, MIN latency (40M build co-running so absolute ms is inflated but the DELTA is the signal).
set -uo pipefail
...
run_arm() {
  local name="$1"; shift; local log="$OUT/run_1m_csel_${name}_${STAMP}.log"
  echo "=== [$name] $(date) ==="
...
run_arm COUNTING JAVA_TOOL_OPTIONS="-Dlloyd.countingSelect=true"
echo "=== done $(date) ==="
