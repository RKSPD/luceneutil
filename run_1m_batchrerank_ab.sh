#!/usr/bin/env bash
# A/B batched-dot rerank (BulkDotKernel) vs per-candidate uint8DotProduct, on the cached m1.2 index at the
# current best op point (np40/bn1750). Both reuse the index (search-time flag). Recall must be identical
# (bit-identical dot); the question is latency. 1000 q x3, MIN lat.
set -uo pipefail
...
run_arm() {
  local name="$1"; local opts="$2"; local log="$OUT/run_1m_brr_${name}_${STAMP}.log"
  echo "=== [$name] $(date) ==="
...
run_arm BATCH  ""
run_arm PERCAND "-Dlloyd.noBatchRerank=true"
echo "=== done $(date) ==="
