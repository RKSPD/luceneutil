#!/usr/bin/env bash
# A/B dedup-in-rerank (fold docId dedup into the read loop) vs the scattered-readInt pre-pass. Reuses cached
# index, np40/bn1750, counting-select on, spill>0 so dedup actually runs. Recall must be identical.
set -uo pipefail
...
run_arm() {
  local name="$1"; local opts="$2"; local log="$OUT/run_1m_dd_${name}_${STAMP}.log"
  echo "=== [$name] $(date) ==="
...
run_arm INLOOP  ""
run_arm PREPASS "-Dlloyd.noDedupInRerank=true"
echo "=== done $(date) ==="
